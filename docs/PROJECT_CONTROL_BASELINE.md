...
- 最终汇总在可用 `output_schema` 时走 `final_analysis` JSON Schema，并设置 `max_output_tokens=4500`，避免自由文本长生成拖垮当前兼容网关连接。
- 本地瘦身目标已从约 2.8 万字符降到约 1.8 万字符，优先保障 100 章级任务能续跑完成；代价是最终汇总素材更摘要化。
- 最终汇总保存前会做基础质量闸门：当 3 章以上任务返回 `N/A`、空摘要且空 `items` 等明显占位结果时，任务标记失败并保留断点续跑，不把占位结果写成完成态。
