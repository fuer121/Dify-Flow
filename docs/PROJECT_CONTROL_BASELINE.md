...
最后更新：2026-05-20 00:48，Asia/Shanghai
...
- 对系统性上游错误提前停止任务
- 对大体量最终汇总自动启用最终 Prompt 感知的结构化分批压缩汇总，降低单次请求体规模和超时风险。
- 压缩阶段会校验章节覆盖完整性；覆盖不完整时直接失败，避免继续生成不可靠最终汇总。
- 压缩汇总素材阶段再次观察到 `This operation was aborted` 后，当前策略改为更小压缩批次，并对汇总阶段瞬时错误做有限重试。
