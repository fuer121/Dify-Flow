...
- 当前 VPN/内网地址：`http://10.8.11.58:5184/`
- 当前 Git 分支：`codex/Prompt-Design`
- 当前分支基线：`2691f6f Merge pull request #4 from fuer121/codex/upgrade01`
- 当前本地 `main` 已对齐 `origin/main`。
...
- `OPENAI_PROXY_URL=http://127.0.0.1:7897` 当前已启用，用于让 OpenAI 长请求走本机代理出口。
  - 影响：只改变后端访问 OpenAI 兼容接口的网络路径，不改变模型、Prompt、L1/L2、数据库或分析逻辑。
  - 风险：如果本机代理关闭、端口变化或线路不稳定，所有 OpenAI 请求都会失败；回滚方式是把该变量清空并重启服务。
...
- `GET /api/tasks` 当前返回空列表，没有导入、L1、L2 或分析任务在当前进程内运行。

当前部署验证：

- 当前正式服务运行版本：`codex/Prompt-Design` 工作区未提交版本，已包含书籍级 Prompt 管理重构
- `http://127.0.0.1:5184/api/config` 正常。
- `http://172.16.75.46:5184/api/config` 正常。
- `http://10.8.11.58:5184/api/config` 正常。
...
- 共 3 条分析 Prompt
- 分类：`剑来` 2 条，`通用` 1 条
- 本轮迁移后，分析 Prompt 会按 `prompt_groups.book_id` 归属到具体书籍；无法匹配书名或书籍 ID 的历史 Prompt 会保留为 `book_id=''`，不在按书籍筛选的创建任务流程中展示，后续需要手动迁移。
...
- `/library`：书籍章节库
  ...
  - 基础索引和 L2 类型化事实模块只展示索引 Prompt 状态、覆盖率、用途说明、首章预览和“管理索引 Prompt”跳转
  - 不再在章节库页面编辑 L1/L2 Prompt

- `/prompts`：统一 Prompt 管理台
  - 顶部按书籍切换 Prompt 工作区
  - 支持只创建书籍元数据，不导入章节
  - 左侧维护当前书籍的 L1/L2 索引 Prompt；每本书只有一套当前生效文本
...
说明：`prompt-groups` 路径为历史兼容命名，当前产品语义是“书籍级分析 Prompt”。`book_index_prompts` 存 L1/L2 书籍级索引 Prompt。
...
- 如果当前书籍仍使用默认 L1 Prompt，Hash 继续使用兼容值 `l1-v1-chapter-window-10`，避免已完成逐章索引被误判为过期。
- 如果当前书籍保存了自定义 L1 Prompt，Hash 基于书籍级 Prompt 文本计算，并只影响该书覆盖率过期判断和后续构建任务。
...
- 默认分析路径已改为 `analysis_mode=balanced`：先按章节顺序扫描 L1 路标，命中目标主体、分类词或路标信号后，再读取命中章节的 L2 类型化事实，并按预算复核少量高风险原文章节；L1 未命中时才退回 L2 兜底召回。
...
- `fast_index` 只用 L2 索引，不读取章节原文；`precision` 使用更高复核预算；`full_text` 保留旧的逐章全文精读模式。
...
- 书籍级索引 Prompt 保存在 `book_index_prompts`，和 `books.book_id` 一一对应；每本书只有一套当前生效的 L1 Prompt 和 L2 Prompt。
- `/prompts` 是统一 Prompt 管理台，左侧管理当前书籍的 L1/L2 索引 Prompt，右侧管理当前书籍的分析 Prompt。
...
- 保存自定义书籍级 L1/L2 Prompt 后，Hash 变为基于该书 Prompt 内容计算的 SHA-256；后续构建任务会按新 Hash 跳过、重建或标记过期。
...
- `OPENAI_PROXY_URL=http://127.0.0.1:7897` 已启用，OpenAI 请求会通过本机代理连接 `OPENAI_API_BASE`；该策略用于改善最终汇总长请求在直连网关上的中途断连。
- 对系统性上游错误提前停止任务
...
L2 类型化事实索引用于 1000 万字、1000+ 章场景下的重复分析。它把章节内容拆成可检索事实单元，分类、主体、标签、章节号、重要度和置信度明文保存，事实正文、证据摘记和复核结果加密保存。默认分析使用 `balanced` 召回式分析；全文精读仍保留，但应作为小范围或高保真兜底路径。
...
- 当前正式环境启用 `OPENAI_PROXY_URL=http://127.0.0.1:7897`。
- 如果代理软件未运行、端口变化或线路不稳，OpenAI 请求会整体失败。
- 如果 `OPENAI_API_BASE` 可直连，可临时把 `OPENAI_PROXY_URL=` 清空并重启服务回滚。
- 如果必须使用代理，长任务前先测试代理：
